...
sudo systemd-nspawn -M "$(basename $HOMEWORLD_CHROOT)" --bind $(pwd):/homeworld:norbind -u "$USER" -a -D "$HOMEWORLD_CHROOT" bash -c "cd /homeworld/${ORIG_REL} && gpg-agent --daemon --keep-tty && exec bash"

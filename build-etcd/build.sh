...
FPMOPT=(-s dir -t deb)
FPMOPT+=(-n hyades-etcd -v "${VERSION}" --iteration 1)
FPMOPT+=(--maintainer 'sipb-hyades-root@mit.edu')
FPMOPT+=(--license APLv2 -a x86_64 --url https://github.com/coreos/etcd/)
FPMOPT+=("${HYBIN}/etcd-${VERSION}-linux-amd64.aci=/usr/lib/hyades/" "${HYBIN}/etcd-current-linux-amd64.aci=/usr/lib/hyades/" "${HYBIN}/etcdctl=/usr/bin/etcdctl")

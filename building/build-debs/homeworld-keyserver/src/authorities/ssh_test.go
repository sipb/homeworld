...
const (
	...
	SSH_TEST2_PUBKEY = "ssh-rsa  user2@host2"
	SSH_TEST3_PUBKEY = "ecdsa-sha2-nistp256 AAAAE2VjZHNhLXNoYTItbmlzdHAyNTYAAAAIbmlzdHAyNTYAAABBBEvnwaQ9ql2V6OfGh94Xl0K5BOlozsfZeyMY7GJw7QjtNqPkI+02K5tSjMOzEwAMBFDgOKjBBhEgU7apjjPTkds= user@hyades"
)

func getSSHAuthority(t *testing.T) *SSHAuthority {
	authority, err := LoadSSHAuthority([]byte(SSH_TEST_PRIVKEY), []byte(SSH_TEST_PUBKEY))
	if err != nil {
...
func TestGetSSHPublicKey(t *testing.T) {
	if !bytes.Equal([]byte(SSH_TEST_PUBKEY), getSSHAuthority(t).GetPublicKey()) {
	...
	if err != nil {
		...
		if err != nil {
			...
			if err != nil {
				...
				tempkey, err := parseSingleSSHKey([]byte(SSH_TEST2_PUBKEY))
				if err != nil {
					t.Error(err)
...
func TestMismatchSSHKeys(t *testing.T) {
	_, err := LoadSSHAuthority([]byte(SSH_TEST_PRIVKEY), []byte(SSH_TEST2_PUBKEY))
	if err == nil {
		t.Errorf("Should not be able to create mismatched authority")
	}
	_, err = LoadSSHAuthority([]byte(SSH_TEST_PRIVKEY), []byte(SSH_TEST3_PUBKEY))
	if err == nil {
		t.Errorf("Should not be able to create mismatched authority")
	}
}
...
func TestMissingSSHKey(t *testing.T) {
	_, err := LoadSSHAuthority([]byte(SSH_TEST_PRIVKEY), []byte(SSH_TEST_PUBKEY))
	if err != nil {
		t.Errorf("Should be able to create normal authority")
	}
	_, err = LoadSSHAuthority([]byte{}, []byte(SSH_TEST_PUBKEY))
	if err == nil {
		t.Errorf("Should not be able to create malformed authority")
	}
	_, err = LoadSSHAuthority([]byte(SSH_TEST_PRIVKEY), []byte{})
	if err == nil {
		t.Errorf("Should not be able to create malformed authority")
	}
}
...
func TestMalformedSSHKey(t *testing.T) {
	...
	_, err = LoadSSHAuthority([]byte(SSH_TEST_PRIVKEY), []byte(strings.Replace(SSH_TEST_PUBKEY, "Z", "Y", 1)))
	if err == nil {
		t.Errorf("Should not be able to create malformed authority")
	}
}

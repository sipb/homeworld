...
tar -C "${PROMETHEUS}" -xf "prometheus-${VERSION}.tar.xz" "prometheus-${VERSION}"
mv "${PROMETHEUS}/prometheus-${VERSION}" "${PROMETHEUS}/prometheus"

mkdir "${XBIN}"
go install github.com/prometheus/promu
cd "${PROMETHEUS}/prometheus" && "${GOPATH}/bin/promu" build --prefix="${XBIN}"

echo "prometheus built!"
